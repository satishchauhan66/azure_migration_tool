        ...
        try:
            ...
            if access_token:
                ...
                try:
                    ...
                    # Token connection failed - force a brand-new token from the refresh token
                    # (the first call returns the same in-memory cached token, which SQL just
                    # rejected, so we must bypass that cache), then retry once.
                    if logger:
                        logger.warning(f"Token-based connection failed: {type(conn_err).__name__}: {conn_err}; forcing token refresh and retrying")
                    access_token_2 = None
                    try:
                        from azure_token_cache import force_refresh_token
                        access_token_2 = force_refresh_token(user)
                    ...
                    if access_token_2 and access_token_2 != access_token:
                        ...
                        try:
                            ...
                            if logger:
                                logger.warning(f"Retry with refreshed token also failed: {type(retry_err).__name__}: {retry_err}")
                            conn_err = retry_err
                    raise RuntimeError(
                        "Cached token was rejected or expired, and a silent refresh did not resolve it. "
                        "Use 'Test Connection' (or sign in from another tab) to sign in again, then retry "
                        "(do not cancel the sign-in when it appears).\n"
                        f"Underlying SQL/ODBC error: {conn_err}"
                    ) from conn_err
        except ImportError:
            # MSAL library not available, fall back to interactive authentication
            if logger:
